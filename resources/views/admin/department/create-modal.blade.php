            ...
            <div class="modal-body">
                ...
                <div class="form-group mb-3">
                    ...
                    <select name="parent_id" class="form-control">
                        <option value="">---</option>
                        @forelse(\App\Models\Department::whereNull('parent_id')->get(['id', 'name']) as $department)
                        <option value="{{ $department->id }}" {{ old('parent_id') == $department->id ? 'selected' : null }}>{{ $department->name }}</option>
                        @empty
                        @endforelse
                    </select>
